      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800 p-6 w-full max-w-sm">
            <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-1">{t("delete_title")}</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">{t("delete_body")}</p>
            <div className="flex gap-3">
              ...
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : t("delete_confirm")}
              </button>
            </div>
          </div>
        </div>
